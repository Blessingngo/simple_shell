...
void print_error(info_t *info, char *estr)
{
	_eputs(info->fname);
	_eputs(": ");
	print_s(info->line_count, STDERR_FILENO);
	_eputs(": ");
	_eputs(info->argv[0]);
	_eputs(": ");
	_eputs(estr);
...
/**
 * print_s - function that prints a decimal (integer) number (base 10)
 * @input: the input
...
/**
 * _remove_comments - function that replaces first instance of '#' with '\0'
